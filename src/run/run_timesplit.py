...
def learning(df_train, df_test):
    ...
    for n_fold, (train_idx, val_idx) in enumerate(folds.split(df_train)):
        splited_fold.append((n_fold, (train_idx, val_idx)))
    splited_fold.append((n_folds, (np.arange(len(df_train)), np.arange(len(df_train))[-10000:])))

    final_iteration = 0
    for n_fold, (train_idx, val_idx) in splited_fold:
        print("-----------------------------------")
        ...
        w_params = copy.copy(params)
        if n_fold == n_folds:
            w_params["n_estimators"] = final_iteration * 1.2
            w_params["early_stopping_rounds"] = 10000 # disable
        model = lgb.train(w_params,
                          lgb_train,

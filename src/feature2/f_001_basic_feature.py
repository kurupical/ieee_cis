...
def identify_id(df):
    ...
    df["TEMP__uid2+DT"] = df["TEMP__uid2"].astype(str)+"_"+(df["TEMP__DT_D"]-df["D1"].replace(0, np.nan)).astype(str)
    df["TEMP__uid3+DT"] = df["TEMP__uid3"].astype(str)+"_"+(df["TEMP__DT_D"]-df["D1"].replace(0, np.nan)).astype(str)
